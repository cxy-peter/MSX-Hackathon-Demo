...
function PaperTradingInner() {
  ...
  const hedgeFocusActive =
    selectedAdvancedRoute === 'perp' &&
    (selectedRouteFocusConfig?.id === 'hedge' || selectedRouteFocusConfig?.id === 'combo');
  const hedgeFocusProfile =
    hedgeFocusActive
      ? buildHedgeRouteProfile(selectedProduct, selectedInsight, selectedProductGuide)
      : null;
  const comboHedgeFocusProfile =
    comboFocusActive && hedgeFocusProfile
      ? {
          ...hedgeFocusProfile,
          panelTitle: 'Leverage + hedge training route',
          summary:
            'Run this as a guided training trade: define the sleeve, add a levered hedge leg, then read residual exposure after margin, funding, and liquidation risk.',
          lessons: [
            'The sleeve is the thing being protected; the perp leg is the training hedge, not a second lottery ticket.',
            'The useful number is residual exposure after hedge ratio, funding, and close drag.',
            'Liquidation distance still matters because a hedge can fail if the levered leg is sized too aggressively.'
          ],
          walkthrough: [
            { number: '1', title: 'Load the practice case', detail: 'Use Start case to place the chart on the training entry bar, then buy the sleeve first.' },
            { number: '2', title: 'Jump to hedge day', detail: 'Use Hedge day to move into the risk window and let the desk size a hedge against the sleeve.' },
            { number: '3', title: 'Open the levered hedge', detail: 'Open the short or perp hedge after margin, flash, funding, and liquidation are visible.' },
            { number: '4', title: 'Stop day and unwind', detail: 'Use Stop day to compare hedge-only PnL with hedge + sleeve settlement and residual exposure.' }
          ],
          tradeConfirmCopy:
            'Confirm the training hedge ticket. This combo route starts with a sleeve and adds a levered protection leg, so the confirmation should be read as exposure reduction after funding and margin drag.',
          riskConfirmCopy:
            'This is a leverage + hedge drill. The hedge can reduce net exposure, but liquidation, funding, flash premium, and close drag can still make the protection imperfect.',
          feeSummary:
            'Combo training asks whether protection survives margin, funding, liquidation buffer, flash premium, and close drag after the sleeve is included.',
          autoTitle: 'Auto-unwind combo hedge',
          holdingLabel: 'Combo hedge window',
          targetLabel: 'Combo unwind bar',
          sizeLabel: 'Levered hedge size'
        }
      : hedgeFocusProfile;
  const activeRouteFocusConfig = comboHedgeFocusProfile
    ? {
        ...selectedRouteFocusConfig,
        ...comboHedgeFocusProfile
      }
    : selectedRouteFocusConfig;
  ...
  const optionStrategyPreview = useMemo(() => {
    ...
    return routePracticeCase?.strategyOutcome ||
      estimateOptionStrategyPracticeOutcome({
        product: selectedProduct,
  ...
  function renderShelfLearningCard() {
    return (
      <section className="paper-shelf-learning-card">
        ...
        {comboFocusActive ? (
          <div className="paper-portfolio-combo-card">
