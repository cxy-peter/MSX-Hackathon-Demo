...
const HIDDEN_LEARNING_ROUTE_IDS = new Set(['borrow', 'routing', 'lending']);
const HIDDEN_PERP_FOCUS_IDS = new Set(['combo']);
const HEDGE_TYPE_OPTIONS = [
  { id: 'auto', label: 'Auto type', shortLabel: 'Auto', copy: 'Use the product layer default so beginners do not have to choose the hedge taxonomy first.' },
  { id: 'direct', label: 'Direct hedge', shortLabel: 'Direct', copy: 'Use the same listed name, ETF, or closest liquid contract when the wrapper is publicly traded.' },
  { id: 'proxy', label: 'Proxy hedge', shortLabel: 'Proxy', copy: 'Use a liquid proxy when the protected sleeve is private or cannot be shorted directly.' },
  { id: 'exit', label: 'Exit hedge', shortLabel: 'Exit', copy: 'Use cash, redemption, or liquidity planning to reduce forced-sale risk instead of pretending a perfect short exists.' },
  { id: 'basket', label: 'Basket hedge', shortLabel: 'Basket', copy: 'Use a basket or factor overlay when the protected exposure is a strategy rather than one ticker.' }
];
const HEDGE_TYPE_LABELS = Object.fromEntries(HEDGE_TYPE_OPTIONS.map((option) => [option.id, option.label]));
const TRADE_ROUTE_DEFINITIONS = [
  {
    id: 'spot',
    label: 'Spot',
    title: 'Spot = buy wrapper exposure, then sell it',
    copy: 'You own the asset wrapper or receipt exposure. The beginner loop is BUY -> hold -> SELL, with fees, spread, and liquidity shown before net PnL.'
  },
  {
    id: 'perp',
    label: 'Perp',
    title: 'Perp = synthetic long / short contract exposure',
    copy: 'You do not own the underlying. You post margin to open long or short notional, then manage funding, liquidation buffer, and close discipline.'
  }
];
...
function buildFlashLiquidityQuoteRows({
  ...
  const reserveBackedCap = roundNumber(
    FLASH_TICKET_RESERVE_RATE > 0 ? Math.max(0, Number(availableCash || 0)) / FLASH_TICKET_RESERVE_RATE : safeTradeAmount,
    2
  );
  ...
  const generalCap = roundNumber(
    Math.min(reserveBackedCap, generalSupportCap),
    2
  );

  return FLASH_LIQUIDITY_QUOTE_LANE_META.map((lane) => {
    const draftNotional = roundNumber(Math.max(0, Number(draftQuotes?.[lane.id] || 0)), 2);
    const appliedNotional = roundNumber(Math.max(0, Number(appliedQuotes?.[lane.id] || 0)), 2);
    ...
    const maxAvailableNotional = lane.id === 'ticket' ? ticketCap : generalCap;
    const supportCap = lane.id === 'ticket' ? ticketSupportCap : generalSupportCap;
    const copy =
      lane.id === 'ticket'
        ? safeLeverage > 1
          ? 'Purpose is bound to the current leveraged ticket, so the surcharge can be fixed once this wallet signs the leverage intent. This is where onchain or signed intent helps: the route can prove what the credit is for.'
          : 'Purpose is bound to the current ticket, so the route can use the lower attested premium once the wallet signs the order intent.'
        : 'This credit can be reused inside the venue, but the route cannot yet prove every downstream use. It stays more expensive until a contract or restricted subaccount can attest the same leverage-only purpose.';
...
function FlashLoanQuoteModal({
  ...
  if (!open || !product) return null;
  const totalAvailableNotional = roundNumber(
    quoteRows.reduce((sum, row) => sum + Number(row.maxAvailableNotional || 0), 0),
    2
  );
        ...
        <div className="section-head wealth-modal-head">
          <div>
            ...
            <div className="paper-float-modal-copy paper-float-modal-copy-flash">
              ...
              <br />
              In this replay desk, <strong>ticket-bound flash</strong> stays attached to the current leverage route, so the contract-style route can verify that the borrow is being used for leverage inside this screen. Because the use is provable, the risk lane is lower and the fee can be lower too.
              <br />
              <strong>Broad flash</strong> still stays inside the venue, but its purpose is looser until the same leverage-only restriction is provable, so it keeps the higher surcharge.
            </div>
          </div>
          <div className="paper-float-modal-actions">
              ...
              <div className="paper-inline-support-list">
                <div className="paper-inline-support-row">
                  <span>
                    ...
                    )} PT and by support at {formatNotional(generalLane?.supportCap || 0)} PT. The smaller cap wins in each lane.
                  </span>
                </div>
              </div>
            </div>
...
function readStoredPaperState(address) {
  return normalizePaperState(readStorageJson(getPaperStateKey(address), defaultPaperState()));
}

function readStoredTradeOutcomeHistory(address) {
  return normalizeTradeOutcomeHistory(readStorageJson(getTradeOutcomeHistoryKey(address), defaultTradeOutcomeHistory()));
}
...
const ADVANCED_REPLAY_ROUTES = [
  ...
  },
  {
    ...
    shortLabel: 'Strategy',
    requiresBaseMint: true,
    description: 'Use template-style overlays and structured legs to compare payoff logic, collateral usage, and drag.',
    lessons: [
      'Borrowing turns a cash-like position into a leveraged one even if the chart looks stable.',
      'Collateral quality, oracle speed, and liquidation penalty decide the real downside.',
      'Borrow loops are useful only when the spread after fees stays positive.'
    ]
  },
  {
    id: 'routing',
...
const REPLAY_ROUTE_UI = {
  ...
  perp: {
    ...
    walkthrough: [
      ...
      },
      {
        ...
        detail: 'Open the long or short leg only after the ticket is sized. The route then asks for trade confirm, route-risk confirm, and wallet signature.'
      },
      {
  ...
  borrow: {
    glyph: 'BR',
    actionTag: 'PLEDGE / BORROW',
    helperLabel: 'Strategies',
    walkthrough: [
      {
        label: '1. Pledge the calmer sleeve',
        detail: 'Use collateral mode before thinking about recursive exposure.'
      },
      {
        label: '2. Borrow against it',
        detail: 'Watch borrow carry, liquidation buffer, and how much spread is really left.'
      },
      {
        label: '3. Add advanced templates after unlock',
        detail: 'Collateral and flash-style templates stay inside the route template menu until both replay tasks are completed.'
      }
    ]
  },
  routing: {
...
const REPLAY_ROUTE_FOCUS_OPTIONS = {
  perp: [
    ...
    },
    {
      ...
      summary: 'Treat the perp leg as insurance for an existing spot, treasury, or tokenized-equity sleeve. The goal is not max upside, but smoother downside and cleaner exit discipline.',
      lessons: [
        'A hedge should reduce net exposure, not secretly become a second speculative bet.',
      ...
      walkthrough: [
        { number: '1', title: 'Click the risk window bar', detail: 'Anchor the hedge on the bar where the underlying sleeve starts to look vulnerable.' },
        { number: '2', title: 'Set hedge notional + optional leverage', detail: 'Size the hedge to offset risk, then decide whether the hedge should stay low-leverage or use a small boost.' },
        { number: '3', title: 'Open the hedge leg + confirm trade', detail: 'Open the long or short hedge only after the route shows the sizing and risk confirms.' },
        { number: '4', title: 'Close when the hedge window ends', detail: 'Use auto-close or manual close to unwind the hedge once the earnings, event, or drawdown window passes.' }
      ]
    },
    {
      id: 'combo',
  ...
  lending: [
    ...
    },
    {
      ...
      lessons: [
        ...
        'This is one of the most MSX-like yield paths because it connects RWA holdings to onchain liquidity.'
      ]
    },
    {
      id: 'carry',
    ...
    },
    {
      ...
      panelTitle: 'Covered-call income route',
      summary: 'Use listed or tokenized public-market exposure as the base asset, then explain how option premium creates income while capping upside.',
      lessons: [
        'Covered-call income is option premium, not free extra yield on a passive stock sleeve.',
        'Users must see the capped upside and still-real downside at the same time.',
        'This route fits public-market wrappers better than treasury cash management.'
      ]
    },
    {
      id: 'points',
  ...
  borrow: [
    {
      id: 'basket',
      label: 'Basket template',
      panelTitle: 'Basket strategy route',
      summary: 'Use multi-asset templates such as AI leaders, public-market baskets, or barbell mixes instead of pretending every payoff is a one-asset trade.',
      lessons: [
        'A basket is a thesis card first and an execution route second.',
        'Blended drag, rebalance rules, and concentration should stay visible.',
        'This is where anchor + active basket belongs, not inside Earn & yield.'
      ]
    },
    {
      id: 'barbell',
      label: 'Barbell mix',
      panelTitle: 'Barbell strategy route',
      summary: 'Combine a calm sleeve such as treasuries with a high-beta growth sleeve, then inspect how much protection the ballast really gives.',
      lessons: [
        'Barbells smooth risk only if the safe sleeve stays liquid and truly uncorrelated enough.',
        'The user should see how the ballast changes drawdown, not just average return.',
        'This is a clean bridge from wealth-style allocation into strategy overlays.'
      ]
    },
    {
      id: 'collar',
      label: 'Income collar',
      panelTitle: 'Income collar route',
      summary: 'Hold the underlying, sell some upside, and buy some protection so the payoff is visibly smoother but more capped.',
      lessons: [
        'A collar is a payoff design, not a generic yield sleeve.',
        'Users need to see where upside is capped and where downside is still exposed.',
        'This is the natural home for option-protection style education after basic leverage.'
      ]
    },
    {
      id: 'structured',
      label: 'Structured income',
      panelTitle: 'Structured income route',
      summary: 'Use more advanced payoff cards such as dual investment, shark fin, or snowball-like structures, but keep them clearly separate from basic earning routes.',
      lessons: [
...
const PRODUCT_ROUTE_PLAYBOOKS = {
  ...
  strategy: {
    defaultRoute: 'borrow',
    routes: ['spot', 'borrow', 'routing'],
    summary: 'Strategies should read like reusable templates or payoff cards, with the structure and thesis explained before the user worries about execution.'
  },
  ai: {
...
function buildHedgeRouteProfile(product, insight, guide) {
  ...
  if (isPublicLike) {
    ...
    return {
      ...
      statCards: [
        ...
        { label: 'Expected haircut', value: '0.5% to 2.0%', copy: 'Most pain comes from spread and wrapper drag, not a forced private-market discount.' },
  ...
  if (isPrivateLike) {
    ...
    const haircutBand = Number(guide?.lockupDays || 0) >= 180 ? '10% to 20%' : '6% to 12%';
  ...
  if (isRwaLike) {
    ...
    const haircutBand = lane === 'funding' ? '0.2% to 1.5%' : '1.0% to 4.0%';
    ...
    return {
      ...
      diligence: {
        ...
        pillLabel: 'Exit-led',
        bullets: [
          'Treat RWA hedge mode as exit, discount, and liquidity management first.',
          'Rate or credit proxies can reduce some sensitivity, but cannot force a redemption window to open.',
          'If the queue widens or the issuer side slows down, haircut risk still remains on the sleeve.',
          'This is where stress testing and redemption monitoring are often more useful than extra leverage.'
        ],
        footer:
          'For RWA-like sleeves, the biggest risk is often that the user cannot exit cleanly at the moment they want to.'
      },
      ideaCopy:
  ...
  return {
    ...
    statCards: [
      ...
      { label: 'Expected haircut', value: '1.0% to 5.0%', copy: 'Drag shows up through rebalance, spread, and route complexity rather than only one ticket fee.' },
...
function buildReplayAchievements({
  ...
  return [
    ...
    },
    {
      ...
      activityLabel: 'Leverage + hedge path',
      coverCopy: 'Hedge path learned',
      requirement: 'Choose a protected sleeve, pick a hedge type and ratio, apply the suggested size, then open a short / perp hedge.',
      reward: 'Shows the user understands that hedging is for reducing net exposure, not adding a second speculative bet.',
      unlocked: onboardingReady && hedgeRouteUsed,
      detail: hedgeRouteUsed
        ? 'A hedge sizing flow and perp hedge leg were completed in the tutorial.'
        : 'Use the Hedge focus under Perp: protect a sleeve, choose Direct / Proxy / Exit / Basket, set 25-100% ratio, apply suggested size, then open the hedge.'
    }
  ];
}
...
function PaperTradingInner() {
  ...
  const [hedgeTypeOverride, setHedgeTypeOverride] = useState('auto');
  ...
  useEffect(() => {
    setProgressState(
      readStorageJson(progressStorageKey, {
        viewedRiskCards: [],
        guideCompleted: false,
        quizCompleted: false,
        paperTradesCompleted: 0,
  ...
  useEffect(() => {
    writeStorageJson(paperStateKey, normalizePaperState(paperState));
  }, [paperState, paperStateKey]);
  ...
  const shelfLeaderboardRows = useMemo(() => {
    const baseRows = filteredProducts.map((product) => {
      ...
      const aiScore = clamp(Math.round(Number(insight.diligenceScore || 0)), 0, 100);
  ...
  const hedgeRouteUsed = Boolean(progressState.hedgeLessonCompleted) || Boolean(replayOnchainStateById[10]?.onchainClaimed);
  ...
  const advancedRouteUnlockCopy = REPLAY_DEVELOPER_MODE
    ...
    : advancedRoutesUnlocked
      ? 'Task 1 and Task 2 are both completed, so perp, lending, borrow, and routing tutorials are now open.'
  ...
  const advancedActivityUnlockCopy = REPLAY_DEVELOPER_MODE
    ...
    : advancedActivityEnabled
      ? 'Task 1 and Task 2 are both completed, so timed exits, route templates, DeFi actions, and guided short setup are available.'
        ...
        : selectedRewardTask.id === 8
          ...
          : selectedRewardTask.id === 9
            ...
            : [
                {
                  id: 'hedge-frame',
                  title: 'Protected sleeve + ratio staged',
                  ...
                  copy: 'Choose the protected sleeve, hedge type, and 25 / 50 / 75 / 100% ratio. Then apply the suggested size.'
                },
                {
                  id: 'hedge-leg',
                  title: 'Protective perp opened',
                  ...
                  interactive: true,
                  onClick: () => scrollToRef(tradeDeskRef),
                  copy: 'Open the hedge leg only after the before / after / residual exposure cards explain what is being reduced.'
                }
              ]
    : [];
  ...
  const timedExitAnchorIndex = Math.max(0, Math.min(selectedView?.cursor || 0, Math.max(0, (selectedView?.bars?.length || 1) - 1)));
  ...
  const routeMarginBufferRate = getPerpMarginBufferRate(Number(tradeAmount || 0), routeLeverageMultiple);
  const routeInitialMarginRate = getPerpInitialMarginRate(Number(tradeAmount || 0), routeLeverageMultiple);
  const routeMaxWalletBackedNotional =
    leverageRouteActive
      ? getWalletBackedPerpNotionalCap(Number(availableCash || 0), routeLeverageMultiple)
      : roundNumber(Math.max(0, Number(availableCash || 0)), 2);
  const routeRequiredBaseMarginCapital = roundNumber(Math.max(0, Number(tradeAmount || 0) * routeInitialMarginRate), 2);
  const routePostedBaseMarginCapital = leverageRouteActive
    ? roundNumber(Math.min(Number(availableCash || 0), routeRequiredBaseMarginCapital), 2)
  ...
  const routeNotionalShortfall = roundNumber(Math.max(0, Number(tradeAmount || 0) - routeBaseNotional), 2);
  const routeMarginWalletFreeCashBeforeFlash = roundNumber(
    Math.max(0, Number(availableCash || 0) - (leverageRouteActive ? routePostedBaseMarginCapital : 0)),
    2
  );
    ...
    () =>
      leverageRouteActive
        ? buildFlashLiquidityQuoteRows({
            ...
            routeBaseNotional,
            availableCash: routeMarginWalletFreeCashBeforeFlash,
            wealthDeskState,
  ...
  const flashLoanQuoteMaxNotional = leverageRouteActive
    ? roundNumber(flashLoanQuoteRows.reduce((sum, row) => sum + Number(row.maxAvailableNotional || 0), 0), 2)
    : 0;
  const flashLoanDisplayMaxNotional = leverageRouteActive
    ? roundNumber(flashLoanDisplayRows.reduce((sum, row) => sum + Number(row.maxAvailableNotional || 0), 0), 2)
    : 0;
  ...
  const routeEffectiveTicketNotional = leverageRouteActive
    ? roundNumber(routeBaseNotional + flashLoanAppliedTicketNotional, 2)
    : roundNumber(Math.max(0, Number(tradeAmount || 0)), 2);
  const flashLoanRemainingShortfall = leverageRouteActive
    ? roundNumber(Math.max(0, Number(tradeAmount || 0) - (routeBaseNotional + flashLoanAppliedTicketNotional)), 2)
    : 0;
  ...
  const routeFlashReserveCapital = leverageRouteActive
    ? roundNumber(flashLoanAppliedTicketNotional * FLASH_TICKET_RESERVE_RATE, 2)
    : 0;
  ...
  const routeMarginWalletFreeCash = roundNumber(
    Math.max(0, Number(availableCash || 0) - (leverageRouteActive ? routeMarginCapital : 0)),
    2
  );
  const routeMaxAttestedNotional = leverageRouteActive
    ? roundNumber(routeMaxWalletBackedNotional + Number(displayTicketFlashLane?.maxAvailableNotional || 0), 2)
    : routeMaxWalletBackedNotional;
  const routeMaxVenueNotional = leverageRouteActive
    ? roundNumber(routeMaxWalletBackedNotional + flashLoanDisplayMaxNotional, 2)
    : routeMaxWalletBackedNotional;
  const routeWalletBackedSetupMarginRate =
    leverageRouteActive && routeMaxWalletBackedNotional > 0
      ? roundNumber(Math.max(0, Number(availableCash || 0)) / routeMaxWalletBackedNotional, 4)
  ...
  const selectedTradeAmountMaxLabel = leverageRouteActive
    ? tradeAmountMaxMode === 'ticket'
      ...
      : tradeAmountMaxMode === 'venue'
        ...
        : 'Wallet-only max'
    : 'Available cash';
  const tradeAmountFieldLabel = hedgeFocusActive ? 'Hedge ticket size' : 'Paper notional';
  ...
  const hedgeLiveSleeveNotional = hedgeHasMeaningfulOpenSleeve
    ...
    : 0;
  const hedgePreviewSleeveMax = roundNumber(Math.max(0, Number(routeMaxWalletBackedNotional || availableCash || 0)), 2);
  const hedgeHasPaperNotionalBase =
    hedgeFocusActive &&
    Boolean(replayFocus.lockedBar || replayFocus.bar) &&
    Number(hedgePreviewSleeveNotional || 0) >= MIN_PAPER_TRADE;
  ...
  const hedgeProtectedSleeveNotional = hedgeHasPaperNotionalBase
    ? roundNumber(Math.max(0, Number(hedgePreviewSleeveNotional || 0)), 2)
    : hedgeLiveSleeveNotional;
  const hedgeSizingBasisLabel = hedgeHasPaperNotionalBase ? 'Paper notional base' : 'Protected sleeve notional';
  const hedgeSizingCapLabel = hedgeHasPaperNotionalBase ? 'Paper-notional cap' : 'Protected sleeve cap';
  ...
  const selectedTradeAmountMaxValue = roundNumber(
    Math.max(
      ...
      hedgeFocusActive && hedgeSizingReady
        ? Math.min(selectedTradeAmountMaxRawValue, hedgeTargetCap)
        : selectedTradeAmountMaxRawValue
    ),
    2
  );
  const selectedTradeAmountLaneCapActive =
    hedgeFocusActive &&
    hedgeSizingReady &&
    selectedTradeAmountMaxRawValue > selectedTradeAmountMaxValue + 0.01;
  ...
  const tradeAmountSliderMax = selectedTradeAmountMaxValue;
  ...
  const hedgePlannedRatio = hedgeSizingReady && hedgeProtectedSleeveNotional > 0
    ? roundNumber(hedgePlannedTicketNotional / hedgeProtectedSleeveNotional, 4)
    : 0;
  const hedgeActualRatio = hedgeSizingReady && hedgeProtectedSleeveNotional > 0 && hedgeLiveTicketNotional > 0
    ...
    : 0;
  const hedgeSuggestedPctOfSleeve = hedgeSizingReady && hedgeProtectedSleeveNotional > 0
    ? roundNumber(Math.min(1, hedgeSuggestedNotional / hedgeProtectedSleeveNotional) * 100, 1)
    : 0;
  const hedgeTargetPctOfSleeve = hedgeSizingReady && hedgeProtectedSleeveNotional > 0
    ? roundNumber(Math.min(1, hedgePlannedTicketNotional / hedgeProtectedSleeveNotional) * 100, 1)
    : 0;
  const hedgeCurrentPctOfSleeve = hedgeSizingReady && hedgeProtectedSleeveNotional > 0
    ? roundNumber(Math.min(1, hedgeLiveTicketNotional / hedgeProtectedSleeveNotional) * 100, 1)
    : 0;
  const hedgeTargetProgressPct = hedgeSizingReady && hedgeSuggestedNotional > 0
    ? roundNumber((hedgePlannedTicketNotional / hedgeSuggestedNotional) * 100, 1)
    : 0;
  const hedgeGapToTarget = hedgeSizingReady
    ? roundNumber(Math.abs(hedgeSuggestedNotional - hedgePlannedTicketNotional), 2)
    : 0;
  ...
  const hedgeExcessNotionalValue = hedgeSizingReady
    ? roundNumber(Math.max(0, hedgeLiveTicketNotional - hedgeSuggestedNotional), 2)
    : 0;
  const hedgeStatus = !hedgeFocusActive
    ...
    : !hedgeSizingReady
      ...
      : hedgeSuggestedNotional <= 0
        ...
        : !activePerpLeg
          ? 'Target staged'
          : Math.abs(hedgeLiveTicketNotional - hedgeSuggestedNotional) <= Math.max(25, hedgeSuggestedNotional * 0.05)
            ? 'On target'
            : hedgeLiveTicketNotional < hedgeSuggestedNotional
              ? 'Under-hedged'
              : 'Over-hedged';
  ...
  const hedgeAfterPreviewExposureValue = hedgeSizingReady
    ? roundNumber(Math.max(0, hedgeProtectedSleeveNotional - Math.min(hedgeProtectedSleeveNotional, hedgePlannedTicketNotional || hedgeSuggestedNotional)), 2)
    : 0;
    ...
    ? [
        ...
        },
        {
          ...
          copy: activePerpLeg
            ...
            : 'Preview uses the staged ticket. Press Use suggested size to line it up with the ratio.',
  ...
  const hedgeWorkflowCards = [
    {
      step: '1',
      title: 'Protected sleeve',
      value: hedgeSizingReady ? `${formatNotional(hedgeProtectedSleeveNotional)} PT` : selectedProduct.ticker,
      copy: hedgeProtectedSleeveLabel || 'Choose the product or live sleeve first.'
    },
    {
      step: '2',
      title: 'Hedge type',
      value: effectiveHedgeTypeOption?.shortLabel || HEDGE_TYPE_LABELS[effectiveHedgeType] || 'Auto',
      copy: effectiveHedgeTypeOption?.copy || 'Direct, Proxy, Exit, and Basket describe what kind of protection is actually available.'
    },
    {
      step: '3',
      title: 'Hedge ratio',
      value: formatPercent(Number(hedgeRatio || 0), 0),
      copy: 'Use 25 / 50 / 75 / 100% before thinking about advanced sizing.'
    },
    {
      step: '4',
      title: 'Suggested size',
      value: hedgeSizingReady ? `${formatNotional(hedgeSuggestedNotional)} PT` : '--',
      copy: 'This is the ticket size the route will apply to the short / perp leg.'
    }
  ];
  const hedgeSecondaryMetricCards = hedgeSizingReady
    ? [
        { label: 'Open hedge ticket', value: `${formatNotional(hedgeLiveTicketNotional)} PT` },
        ...
        {
          label: hedgeStatus === 'Over-hedged' ? 'Excess hedge' : 'Gap to suggested',
  ...
  const timedExitLeveragedSnapshot = buildLeveragedReplaySnapshot({
    ...
    entryBar: timedExitAnchorBar,
    targetBar: timedExitTargetBar,
    holdingDays: timedExitActualHoldingDays,
    targetNotional: routeEffectiveTicketNotional,
    flashLoanAmount: flashLoanAppliedTicketNotional,
    flashLoanFee: flashLoanPremiumEstimate
  });
  const replayFocusLeveragedSnapshot = buildLeveragedReplaySnapshot({
    direction: leverageDirection,
    marginCapital: routePostedBaseMarginCapital,
    leverage: routeLeverageMultiple,
    entryBar: replayFocus.lockedBar || replayFocus.bar,
    targetBar: replayFocus.bar,
    holdingDays:
      replayFocus.lockedBar?.ts && replayFocus.bar?.ts
        ? Math.max(0, getHoldingDays(replayFocus.lockedBar.ts, replayFocus.bar.ts))
        : 0,
    targetNotional: routeEffectiveTicketNotional,
    flashLoanAmount: flashLoanAppliedTicketNotional,
    flashLoanFee: flashLoanPremiumEstimate
  ...
  async function ensurePerpRiskApproval({
    ...
    try {
      const signature = await signMessageAsync({
        message: [
          'MSX replay leverage risk acknowledgement',
          `Wallet: ${address}`,
          ...
          `Reason: ${reason}`,
          'If credit stays ticket-bound or contract-attested, the route can justify the lower fee lane. Broad flash credit stays more expensive until the same leverage-only purpose is provable.',
  ...
  async function ensurePerpTradeSignature({
    ...
    try {
      await signMessageAsync({
        message: [
          'MSX replay leverage trade confirmation',
          `Wallet: ${address}`,
          `Product: ${selectedProduct.ticker}`,
  ...
  }, [leverageRouteActive, selectedProductId]);

  useEffect(() => {
    if (!hedgeFocusActive || !hedgeSizingReady) return;
    if (Number(tradeAmount || 0) <= selectedTradeAmountMaxValue + 0.01) return;
    const clampedAmount = Math.max(0, roundNumber(selectedTradeAmountMaxValue, 2));
    setTradeAmount(clampedAmount);
    setTradeAmountInput(String(clampedAmount));
    setTradeAmountMaxApplied(false);
  }, [hedgeFocusActive, hedgeSizingReady, selectedTradeAmountMaxValue, tradeAmount]);
  ...
  useEffect(() => {
    ...
    } else if (flashLoanAppliedTicketNotional > flashLoanQuoteMaxNotional) {
  ...
  useEffect(() => {
    ...
    setFlashLoanTicketConfirmOpen(false);
    flashLoanQuoteContextRef.current = '';
    setFeedback('Paper notional or leverage changed. Flash quote was cleared, so please requote for this ticket.');
  ...
  function handleHedgePreviewSleeveBlur() {
    ...
    const normalizedAmount = Math.max(0, roundNumber(Number(hedgePreviewSleeveInput || 0), 2));
  ...
  function handleApplyHedgeSuggestedSize() {
    if (!hedgeFocusActive || !hedgeSizingReady || hedgeSuggestedNotional <= 0) return;
    setTradeAmountMaxApplied(false);
    setTradeAmount(hedgeSuggestedNotional);
    setTradeAmountInput(String(hedgeSuggestedNotional));
  ...
  function handleTradeAmountMax() {
    ...
    const nextMax = roundNumber(Math.max(0, Number(selectedTradeAmountMaxValue || 0)), 2);
  ...
  function handleTradeAmountMaxModeChange(nextMode) {
    ...
    if (leverageRouteActive && nextMode !== 'wallet') {
      setFeedback(
        nextMode === 'ticket'
          ? 'Attested flash range selected. The flash quote confirm screens will appear when you open long or short.'
          : 'Broad flash range selected. The flash quote confirm screens will appear when you open long or short.'
      );
    }
  }
  ...
  function finalizeTradeOutcome({
    ...
    if (compensationGranted > 0) {
      setPaperState((current) => ({
        ...current,
        cash: roundNumber(current.cash + compensationGranted, 2)
      }));
    }
  ...
  function executeReplayTrade({
    ...
    const filledUnits = roundNumber(fillNotional / price, 6);
    ...
    setPaperState((current) => {
      ...
      const unitsToSell = Math.min(livePosition.units, filledUnits);
      const remainingUnits = roundNumber(Math.max(0, livePosition.units - unitsToSell), 6);
      ...
      sellOutcomeSummary = {
        ...
        actionLabel: outcomeActionLabel || (forceSellAll ? 'Auto-sell' : 'Sell'),
        ts: tradeBar.ts
      };

      return {
        ...current,
        cash: roundNumber(current.cash + netProceeds, 2),
        realizedPnl: roundNumber(current.realizedPnl + realizedPnl, 2),
  ...
  function handlePlaceTrade(side) {
    ...
    const focusedUnitsPreview = focusedPrice > 0 ? roundNumber(Number(tradeAmount || 0) / focusedPrice, 6) : 0;
    const customFeedback = focusedTradeBar
      ? side === 'buy'
        ? `Buy confirmed on ${formatReplayDate(focusedTradeBar.ts, selectedView?.interval)} at ${formatPrice(
            focusedPrice
          )}. Ticket ${formatNotional(tradeAmount)} PT is about ${formatUnits(focusedUnitsPreview)} units before route costs.`
    ...
    if (side === 'buy' && (!Number.isFinite(Number(tradeAmount)) || Number(tradeAmount) < MIN_PAPER_TRADE)) {
      setFeedback(`Minimum replay trade is ${MIN_PAPER_TRADE.toLocaleString()} PT.`);
      return;
    }
    ...
    if (side === 'buy') {
      setPendingBuyTrade({
        ...
        tradeInterval: selectedView?.interval,
        requestedNotional: Number(tradeAmount || 0),
  ...
  function handleTimedExitReplay() {
    ...
    if (selectedAdvancedRoute === 'perp') {
      if (!activePerpLeg) {
        setFeedback('Open long or open short first so the leverage route has a live leg to close.');
        return;
      }
      ...
      const customFeedback = `Auto-close ${activePerpLeg} after about ${executedHoldingDays}D: ${formatReplayDate(
        anchorBar?.ts,
      ...
      const leveragedTradeOutcome = finalizeTradeOutcome({
        product: selectedProduct,
        routeId: 'perp',
        actionLabel: `Auto-close ${activePerpLeg}`,
        tradeTs: targetBar?.ts,
        pnl: leveragedSnapshot.netPnl,
        exitValue: roundNumber((leveragedSnapshot.netExitValue || 0) + routePostedFlashReserveCapital, 2)
      });
  ...
  function handleOpenFlashLoanConfirm() {
    ...
    if (!Number.isFinite(Number(tradeAmount)) || Number(tradeAmount) <= 0) {
      setFeedback('Set a paper notional above 0 PT first, then confirm that ticket before quoting flash liquidity.');
      return;
    }
  ...
  function handleFlashLoanPreview() {
    ...
    if (!Object.keys(flashLoanDraftQuotes).length) {
      const defaultDesiredBoost = roundNumber(
        Math.max(
          ...
          flashLoanAppliedTicketNotional,
          Math.min(Math.max(0, Number(tradeAmount || 0)), flashLoanQuoteMaxNotional)
        ),
        2
      );
    ...
    setFeedback(
      Number(flashLoanQuoteCaps.ticket || 0) > 0 || Number(flashLoanQuoteCaps.general || 0) > 0
        ? 'Flash quote opened. You can use it either to cover a margin gap or to add extra ticket notional above the wallet-backed setup.'
  ...
  function handleFlashLoanQuoteChange(laneId, nextValue) {
    const laneCap = Number(flashLoanQuoteCaps?.[laneId] || flashLoanQuoteMaxNotional || 0);
  ...
  function handleFlashLoanQuoteMax(laneId, maxValue) {
    handleFlashLoanQuoteChange(laneId, roundNumber(Math.max(0, Number(maxValue || 0)), 2));
  }
  ...
  async function handleConfirmFlashLoanQuote() {
    ...
    flashLoanQuoteContextRef.current = currentFlashLoanContextKey;
    setFeedback(
      `Flash quote attached: ${formatNotional(totalApplied)} PT notional. Ticket-bound credit prices first, while broad flash credit keeps the higher surcharge until the venue or contract can prove the same leverage-only purpose. Wallet cash stays unchanged because the added notional stays route-bound instead of becoming spendable paper cash.`
    );
  ...
  async function completePerpShortcut(direction) {
    ...
    const tradeSigned = await ensurePerpTradeSignature({
      ...
      flashNotional: nextLeg ? flashLoanAppliedTicketNotional : 0,
      reason: nextLeg ? `open a ${direction} leveraged leg` : `close the ${direction} leveraged leg`
    });
    ...
    const leveragedTradeOutcome =
      !nextLeg && closingSnapshot
        ? finalizeTradeOutcome({
            product: selectedProduct,
            routeId: 'perp',
            actionLabel: `Close ${direction}`,
            tradeTs: replayFocus.bar?.ts || replayFocus.lockedBar?.ts,
            pnl: closingSnapshot.netPnl,
            exitValue: roundNumber((closingSnapshot.netExitValue || 0) + routePostedFlashReserveCapital, 2)
          })
        : null;
    setFeedback(
      nextLeg
        ...
        : `${hedgeFocusActive ? `Perp ${direction} hedge is now closed.` : `Perp ${direction} tutorial is now closed.`} You can reopen it or switch direction any time.` +
  ...
  async function handlePerpShortcut(direction) {
    if (!advancedActivityEnabled) {
      setFeedback(advancedActivityUnlockCopy);
      setSelectedReplayPanel('desk');
      return;
    ...
    if (hedgeFocusActive && nextLeg && !hedgeSizingReady) {
      setFeedback('Lock a replay bar and set the paper notional base first so hedge mode can size the protective leg before the hedge opens.');
      return;
    }
  ...
  async function confirmPerpTradeModal() {
    ...
    setPendingPerpRiskConfirm({
      ...
      flashAmount: queuedTrade.action === 'open' ? flashLoanAppliedTicketNotional : 0,
    ...
    switch (panelId) {
          ...
          <div className="paper-desk-grid" ref={tradeDeskRef}>
            ...
            <div className="paper-desk-pane paper-desk-pane-routes">
              ...
              <div className="wealth-inline-note paper-inline-note">
                ...
                Leverage & hedging, Earn & yield, Strategies, and Automation / AI tutorials without pretending they are already live execution tools.
              </div>
            </div>
      ...
      case 'diligence':
        return (
          <>
            <div className="paper-panel-pill-row">
              <span className="pill risk-low">AI score {selectedInsight.diligenceScore}/100</span>
            </div>

            <div className="starter-reasons">
              {selectedProductGuide.stressScenarios.map((scenario) => (
                <div className="reason-card" key={scenario.label}>
                  <div className="entry-title">{scenario.label}</div>
                  <div className="entry-copy">{scenario.impact}</div>
                </div>
              ))}
            </div>

            <div className="wealth-inline-note paper-inline-note">
              Higher scores mean the product structure, reserve logic, and path to user net return are easier to inspect, not that the price can only go up. These shocks are the ones the replay desk now stresses before it trusts a yield or payoff claim.
            </div>
          </>
        );
      case 'automation':
  ...
  function renderCompactReplayCard(panelId) {
    if (panelId === 'diligence') {
      const diligenceBreakdown = [
        {
          label: 'Structure',
          score: Math.max(58, Math.min(96, selectedInsight.diligenceScore - 4)),
          copy: 'How clearly the wrapper, legal path, and payoff logic can be explained.'
        },
        {
          label: 'Underlying proof',
          score: Math.max(54, Math.min(96, 62 + selectedInsight.holdings.length * 7)),
          copy: 'How visible the sleeve, reserve path, and source asset trail are.'
        },
        {
          label: 'Fee clarity',
          score: Math.max(56, Math.min(96, 60 + selectedInsight.feeStack.length * 6)),
          copy: 'Whether route drag, platform cost, and tax lens are easy to inspect.'
        },
        {
          label: 'Execution fit',
          score: Math.max(52, Math.min(96, selectedProduct.lane === 'leverage' ? 63 : selectedProduct.lane === 'private' ? 68 : 78)),
          copy: 'How easily a new user can connect the product to an actual trading action.'
        }
      ];
      const diligencePages = [
          ...
          : []),
        {
          ...
          title: 'Trust score before trade',
          pill: <span className="pill risk-low">AI {selectedInsight.diligenceScore}/100</span>,
          body: (
            <>
              <div className="paper-side-score-grid">
              ...
              <div className="paper-side-score">
                Higher scores mean the structure, reserve trail, and path to user net return are easier to inspect. The actual
                stress checks now sit inside the buy simulation so the user sees them before placing the ticket.
              </div>
            </>
          )
        },
  ...
  const routeActionCopy =
    selectedAdvancedRoute === 'perp'
      ? {
          ...
          primary: hedgeFocusActive ? 'Open long hedge' : 'Open long',
          secondary: hedgeFocusActive ? 'Open short hedge' : 'Open short',
      ...
      : selectedAdvancedRoute === 'lending'
        ...
        : selectedAdvancedRoute === 'borrow'
          ? {
              title: activeRouteFocusConfig?.panelTitle || 'Strategies route',
              primary: 'Build route',
              secondary: 'Stress route',
              copy: activeRouteFocusConfig?.summary || 'Preview collateral, borrow carry, and why a loop only works if the spread survives fee and liquidation drag.'
            }
  ...
  const currentLeverageExitReturn =
    leverageRouteActive && activePerpLeg && replayFocusLeveragedSnapshot
      ? roundNumber((replayFocusLeveragedSnapshot.netExitValue || 0) + routePostedFlashReserveCapital, 2)
      : 0;
  const projectedLeverageExitReturn =
    leverageRouteActive && activePerpLeg && timedExitLeveragedSnapshot
      ? roundNumber((timedExitLeveragedSnapshot.netExitValue || 0) + routePostedFlashReserveCapital, 2)
      : 0;
  const currentLeverageAccountAdjustment =
    leverageRouteActive && activePerpLeg && replayFocusLeveragedSnapshot
      ? roundNumber(currentLeverageExitReturn - routeMarginCapital, 2)
      : 0;
  const projectedLeverageAccountAdjustment =
    leverageRouteActive && activePerpLeg && timedExitLeveragedSnapshot
      ? roundNumber(projectedLeverageExitReturn - routeMarginCapital, 2)
      : 0;
    ...
    selectedAdvancedRoute === 'perp'
      ? hedgeFocusActive
        ...
        : comboFocusActive
          ...
          : `If flash quote is 0, the initial margin has already used the wallet reserve cash. With only ${formatNotional(
              routeMarginWalletFreeCashBeforeFlash
            )} PT left before flash reserve, this route cannot stage extra flash notional yet.`
      : selectedAdvancedRoute === 'lending'
        ...
        : selectedAdvancedRoute === 'borrow'
          ? `${activeRouteFocusConfig?.summary || 'Strategy routes should read like payoff cards.'} Keep the thesis, drag stack, and downside style visible before execution details.`
          : selectedAdvancedRoute === 'routing'
  ...
  function renderReplayDeskCompact() {
    ...
    const primaryActionButtonClass =
      selectedAdvancedRoute === 'perp'
        ? `primary-btn ${activePerpLeg === 'long' ? 'paper-inline-route-action-active' : ''}`.trim()
        : 'primary-btn';
    const secondaryActionButtonClass =
      selectedAdvancedRoute === 'perp'
        ? `primary-btn ${activePerpLeg === 'short' ? 'paper-inline-route-action-active' : ''}`.trim()
        : 'secondary-btn';
  ...
  const timedExitStatus = simulationHoldingDays <= 0
    ...
    : hedgeFocusActive && !hedgeSizingReady
      ...
      : selectedPosition.units <= 0 && !leverageRouteActive
        ? { label: 'Buy first', tone: 'risk-medium' }
      : timedExitRequestedExceedsWindow
          ...
          : !advancedActivityEnabled
            ...
            : timedExitReady
              ...
              : { label: 'Lock', tone: 'risk-medium' };
    const timedExitSummary = !advancedActivityEnabled
      ? advancedActivityUnlockCopy
        : simulationHoldingDays <= 0
          ? hedgeFocusActive
  ...
  const routeCashLabel = leverageRouteActive ? selectedTradeAmountDisplayLabel : 'Available cash';
    const routeCashDisplayValue = leverageRouteActive ? selectedTradeAmountMaxValue : routeDisplayAvailableCash;
    const ticketRouteNotional = roundNumber(routeBaseNotional + flashLoanAppliedTicketNotional, 2);
    ...
    const statPositionLabel = leverageRouteActive
      ? activePerpLeg && replayFocusLeveragedSnapshot
    ...
    const statExitValue = leverageRouteActive
      ? activePerpLeg
        ? replayFocusLeveragedSnapshot?.netExitValue || 0
        : 0
      : replayFocus.snapshotAtBar.netExitValue;
    const latestSelectedTradeExitValue = latestSelectedTrade
      ? roundNumber(
      ...
      : 0;
    const currentTradeExitValue = useTimedExitSummary
      ...
      : leverageRouteActive
        ...
        : selectedPosition.units > 0
          ...
          : latestSelectedTrade?.side === 'sell'
            ? latestSelectedTradeExitValue
            : statExitValue;
    const currentTradeRealizedPnl = leverageRouteActive
      ? 0
      : latestSelectedTrade?.side === 'sell'
        ? roundNumber(Number(latestSelectedTrade.realizedPnl || 0), 2)
        : 0;
    const currentTradeNetPnl = useTimedExitSummary
      ? timedExitEstimatedPnl
      : leverageRouteActive
        ? activePerpLeg
        ...
        : selectedPosition.units > 0
          ...
          : latestSelectedTrade?.side === 'sell'
            ? roundNumber(Number(latestSelectedTrade.realizedPnl || 0), 2)
            : 0;
    const currentTradeExitValueLabel = useTimedExitSummary
      ? leverageRouteActive
        ? 'Auto-close take-home'
        : 'Auto-sell take-home'
      : 'Trade exit value';
    const currentTradeRealizedLabel = 'Trade realized PnL';
    const currentTradeNetLabel = useTimedExitSummary
      ? leverageRouteActive
        ? 'Auto-close est. PnL'
        : 'Auto-sell est. PnL'
      : 'Trade net PnL';
  const focusedTradeModeCopy =
    ...
    return (
      ...
      >
          ...
          <div className="paper-inline-desk-route-main">
            ...
            <div className="paper-inline-desk-copy-row">
              ...
              <div className="paper-inline-desk-copy-actions">
                ...
                {selectedAdvancedRoute === 'perp' ? (
                  <div className="paper-inline-help-pill">
                    <span>Hedge idea</span>
                    <div className="paper-inline-cash-tooltip">
                      {hedgeFocusProfile?.ideaCopy ||
                        'Keep the spot sleeve or tokenized stock exposure, then short the same underlying with a perp for part or all of that delta. Funding and basis drag keep accruing while the hedge stays open, so the holding period still matters.'}
                    </div>
                  </div>
                ) : null}
              </div>
        ...
        <div className="paper-route-definition-strip">
          {TRADE_ROUTE_DEFINITIONS.map((definition) => (
            <div
              key={definition.id}
              className={`paper-route-definition-card ${selectedAdvancedRoute === definition.id ? 'active' : ''}`}
            >
          ...
          <div className="paper-inline-desk-panel">
            <div className="paper-inline-desk-controls">
              <div className="paper-inline-desk-top-stack">
                {hedgeFocusActive ? (
                  <label className="paper-inline-route-paper-notional paper-inline-route-paper-notional-inline">
                    <div className="paper-inline-route-paper-notional-head">
                      <span>Paper notional</span>
                      <strong>{formatNotional(hedgePreviewSleeveNotional)} PT</strong>
                    </div>
                    <div className="paper-inline-route-paper-notional-row">
                      ...
                      <div className="paper-inline-route-paper-notional-cap">
                        <span>{selectedTradeAmountCapSummaryLabel}</span>
                        <strong>{formatNotional(selectedTradeAmountMaxValue)} PT</strong>
                      </div>
                    </div>
                    <input
                      type="range"
                      ...
                      step="0.01"
                      value={Math.min(Math.max(0, Number(hedgePreviewSleeveNotional || 0)), Math.max(0, hedgePreviewSleeveMax))}
                    ...
                    <div className="paper-inline-route-paper-notional-copy">
                      {hedgeHasPaperNotionalBase
                        ? hedgeHasMeaningfulOpenSleeve
                          ? `Live sleeve mark: ${formatNotional(hedgeLiveSleeveNotional)} PT. Hedge sizing now follows the paper-notional base instead of the mark-only sleeve.`
                          : 'Set the protected principal first, then stage the hedge ticket against that paper notional.'
                        : hedgeHasMeaningfulOpenSleeve
                          ? `Live sleeve mark: ${formatNotional(hedgeLiveSleeveNotional)} PT. Move the slider to reset the hedge against a larger paper-notional base.`
                          : 'Set the protected principal first, then stage the hedge ticket against that paper notional.'}
                    </div>
                  </label>
                ) : null}
                <label className="wealth-field paper-inline-desk-field">
                  <div className="paper-inline-desk-label-row">
                    ...
                    <div className="paper-inline-cash-pill paper-inline-cash-pill-compact">
                      ...
                      <div className="paper-inline-cash-tooltip">
                        {leverageRouteActive ? <div><strong>{selectedTradeAmountDisplayLabel}</strong>: {formatNotional(selectedTradeAmountMaxValue)} PT</div> : null}
                        {hedgeFocusActive && hedgeSizingReady ? (
                          <div>Hedge target is capped by the sizing base: {formatNotional(hedgeTargetCap)} PT.</div>
                        ) : null}
                        {leverageRouteActive && tradeAmountMaxMode === 'wallet' ? (
                          <>
                            <div>Raw wallet-only max is solved from available cash against the current leverage + margin-buffer schedule.</div>
                            <div>Available cash {formatNotional(availableCash)} PT / setup margin rate {formatPercent(routeWalletBackedSetupMarginRate, 2)}</div>
                            {hedgeFocusActive && hedgeSizingReady ? (
                              <>
                                <div>Raw wallet-only max: {formatNotional(selectedTradeAmountMaxRawValue)} PT.</div>
                                <div>{hedgeSizingCapLabel}: {formatNotional(hedgeTargetCap)} PT.</div>
                                <div>Wallet-only hedge cap = min(raw wallet-only max, {hedgeSizingCapLabel.toLowerCase()}) = {formatNotional(selectedTradeAmountMaxValue)} PT.</div>
                              </>
                            ) : null}
                          </>
                        ) : null}
                        {leverageRouteActive && tradeAmountMaxMode === 'ticket' ? (
                          <>
                            <div>Raw attested flash max = wallet-only max + ticket-bound flash cap.</div>
                            <div>
                            ...
                            {hedgeFocusActive && hedgeSizingReady ? (
                              <>
                                <div>Raw attested flash max: {formatNotional(selectedTradeAmountMaxRawValue)} PT.</div>
                                <div>{hedgeSizingCapLabel}: {formatNotional(hedgeTargetCap)} PT.</div>
                                <div>Attested hedge cap = min(raw attested flash max, {hedgeSizingCapLabel.toLowerCase()}) = {formatNotional(selectedTradeAmountMaxValue)} PT.</div>
                              </>
                            ) : null}
                          </>
                        ) : null}
                        {leverageRouteActive && tradeAmountMaxMode === 'venue' ? (
                          <>
                            <div>Raw broad flash max = wallet-only max + ticket-bound flash cap + broad flash cap.</div>
                            <div>
                            ...
                            {hedgeFocusActive && hedgeSizingReady ? (
                              <>
                                <div>Raw broad flash max: {formatNotional(selectedTradeAmountMaxRawValue)} PT.</div>
                                <div>{hedgeSizingCapLabel}: {formatNotional(hedgeTargetCap)} PT.</div>
                                <div>Broad hedge cap = min(raw broad flash max, {hedgeSizingCapLabel.toLowerCase()}) = {formatNotional(selectedTradeAmountMaxValue)} PT.</div>
                              </>
                            ) : null}
                          </>
                        ) : null}
                        {leverageRouteActive ? <div>Wallet reserve cash after posted margin: {formatNotional(routeDisplayAvailableCash)} PT</div> : null}
                  ...
                  <div className={`paper-inline-trade-amount-row ${leverageRouteActive ? 'with-mode' : ''}`.trim()}>
                    {leverageRouteActive ? (
                  ...
                  <div className="paper-inline-trade-amount-slider">
                    ...
                    {leverageRouteActive ? (
                      <div className="paper-inline-trade-amount-zone-bar" aria-hidden="true">
                        {tradeAmountWalletSegmentPercent > 0 ? (
                          <div className="paper-inline-trade-amount-zone wallet" style={{ width: `${tradeAmountWalletSegmentPercent}%` }}>
                            {hedgeFocusActive ? 'Wallet' : 'Wallet-backed'}
                          </div>
                        ) : null}
                        {tradeAmountAttestedSegmentPercent > 0 ? (
                          <div className="paper-inline-trade-amount-zone ticket" style={{ width: `${tradeAmountAttestedSegmentPercent}%` }}>
                            {hedgeFocusActive ? 'Attested' : 'Attested flash'}
                          </div>
                        ) : null}
                        {tradeAmountBroadSegmentPercent > 0 ? (
                          <div className="paper-inline-trade-amount-zone venue" style={{ width: `${tradeAmountBroadSegmentPercent}%` }}>
                            {hedgeFocusActive ? 'Broad' : 'Broad flash'}
                          </div>
                        ) : null}
                      </div>
                    ) : null}
                  ...
                  {leverageRouteActive && flashLoanAppliedTicketNotional > 0 ? (
                    <div className="paper-inline-note-box paper-inline-route-capacity-note paper-inline-route-capacity-note-applied">
                      Flash stack: {flashLoanAppliedSummaryLabel}. Premium est. <strong className="risk-high">-{formatNotional(flashLoanPremiumEstimate)} PT</strong>. Extra opening reserve posted <strong className="risk-high">-{formatNotional(routeFlashReserveCapital)} PT</strong>. Wallet cash stays unchanged because this added notional stays inside the route, but the reserve still stays posted while the route is staged.
                  ...
                  {hedgeFocusActive ? (
                    <div className="paper-inline-action-card paper-inline-action-card-compact paper-inline-hedge-frame-card">
                      <div className="paper-inline-action-head">
                        <div>
                          <div className="k">Hedge workflow</div>
                          <div className="muted">Pick what is being protected first, then size the hedge from net exposure.</div>
                        </div>
                        <span className="pill risk-low">{hedgeFocusProfile?.diligence?.pillLabel || 'Hedge'}</span>
                      </div>
                      ...
                      <div className="paper-inline-hedge-type-strip">
                        {HEDGE_TYPE_OPTIONS.map((option) => (
                          <button
                            key={option.id}
                            type="button"
                            className={`paper-inline-hedge-type-btn ${hedgeTypeOverride === option.id ? 'active' : ''}`}
                            onClick={() => setHedgeTypeOverride(option.id)}
                          >
                      ...
                      {hedgeSizingReady ? (
                        <>
                          <div className="paper-inline-hedge-progress">
                            <div className="paper-inline-hedge-progress-head">
                              <span>{hedgeSizingBasisLabel}</span>
                              <strong>{formatNotional(hedgeProtectedSleeveNotional)} PT protected</strong>
                            </div>
                            <div className="paper-inline-hedge-progress-bar" aria-hidden="true">
                              <div className="paper-inline-hedge-progress-track" />
                              {hedgeSuggestedPctOfSleeve > 0 ? (
                                <div className="paper-inline-hedge-progress-fill suggested" style={{ width: `${hedgeSuggestedPctOfSleeve}%` }} />
                              ) : null}
                              {hedgeTargetPctOfSleeve > 0 ? (
                                <div className="paper-inline-hedge-progress-fill target" style={{ width: `${hedgeTargetPctOfSleeve}%` }} />
                              ) : null}
                              {hedgeCurrentPctOfSleeve > 0 ? (
                                <div className="paper-inline-hedge-progress-fill current" style={{ width: `${hedgeCurrentPctOfSleeve}%` }} />
                              ) : null}
                            </div>
                            <div className="paper-inline-hedge-progress-legend">
                              <span><i className="suggested" /> Suggested {formatNotional(hedgeSuggestedNotional)} PT</span>
                              <span><i className="target" /> Staged {formatNotional(hedgePlannedTicketNotional)} PT</span>
                              <span><i className="current" /> Open {formatNotional(hedgeLiveTicketNotional)} PT</span>
                            </div>
                          </div>
                          ...
                          <div className="paper-inline-structure-strip paper-inline-structure-strip-compact paper-inline-holding-presets paper-inline-hedge-ratio-strip">
                            {[0.25, 0.5, 0.75, 1].map((ratio) => (
                              <button
                                key={ratio}
                                type="button"
                                className={`ghost-btn compact ${hedgeRatio === ratio ? 'active-toggle' : ''}`}
                                onClick={() => setHedgeRatio(ratio)}
                              >
                            ...
                            <button
                              ...
                              disabled={!hedgeSizingReady || hedgeSuggestedNotional <= 0}
                            >
                              Use suggested size
                            </button>
                          </div>

                          <div className="paper-inline-hedge-secondary-grid">
                            {hedgeSecondaryMetricCards.map((card) => (
                              <div key={card.label} className="paper-inline-hedge-ticket-card">
                                <span>{card.label}</span>
                      ...
                      <div className="paper-inline-support-list">
                        <div className="paper-inline-support-row">
                          ...
                          <strong>{hedgeSuggestedTool}</strong>
                        </div>
                        <div className="paper-inline-support-row">
                          <span>Settlement mode</span>
                          <strong>{hedgeSettlementMode}</strong>
                        </div>
                      </div>
                    </div>
                  ) : null}
                ...
                <div className={`paper-inline-action-card paper-inline-action-card-compact paper-inline-timed-exit-card ${timedExitReady ? 'ready' : 'locked'}`}>
                  <div className="paper-inline-action-head">
          ...
          <div className="paper-inline-preview-panel paper-inline-trade-panel">
            <div className="paper-inline-desk-button-strip compact">
              <button
                ...
                onClick={() => (routeActionsLocked ? handlePlaceTrade('buy') : selectedAdvancedRoute === 'perp' ? handlePerpShortcut('long') : handlePlaceTrade('buy'))}
              ...
              <button
                ...
                onClick={() => (routeActionsLocked ? handlePlaceTrade('sell') : selectedAdvancedRoute === 'perp' ? handlePerpShortcut('short') : handlePlaceTrade('sell'))}
                disabled={selectedAdvancedRoute === 'perp' && isRiskSigning}
              >
                      ...
                      <div className="paper-inline-structure-strip">
                        {[0.3, 0.5, 0.7].map((ratio) => (
                          <button
                            key={ratio}
                            type="button"
                            className={`ghost-btn compact ${routeBufferRatio === ratio ? 'active-toggle' : ''}`}
                            onClick={() => setRouteBufferRatio(ratio)}
                          >
                            {(ratio * 100).toFixed(0)}% LTV
                          </button>
                        ))}
                      </div>
                    ) : null}
            ...
            <div className="paper-chart-stat-grid paper-inline-chart-stats">
              <>
                <div className="guide-chip">
                  ...
                  <div className="v">{statPositionLabel}</div>
                </div>
                <div className="guide-chip">
                  <div className="k">{currentTradeExitValueLabel}</div>
                  <div className="v">{formatNotional(currentTradeExitValue)} PT</div>
                </div>
                <div className="guide-chip">
                  <div className="k">{currentTradeRealizedLabel}</div>
                  <div className={`v ${currentTradeRealizedPnl >= 0 ? 'risk-low' : 'risk-high'}`}>
                    {formatSigned(currentTradeRealizedPnl)} PT
                  </div>
                </div>
                <div className="guide-chip">
                  <div className="k">{currentTradeNetLabel}</div>
                  <div className={`v ${currentTradeNetPnl >= 0 ? 'risk-low' : 'risk-high'}`}>
                    {formatSigned(currentTradeNetPnl)} PT
                  </div>
                </div>
              </>
            </div>
        ...
        <FlashLoanQuoteModal
          ...
          maxBorrowNotional={flashLoanQuoteMaxNotional}
    ...
    return (
      <div className={`paper-replay-leaderboard-card ${compact ? 'compact' : ''}`}>
        ...
        <div className="paper-leaderboard-copy">
            Ranked by total replay return. Each wallet appears once using its best recorded result, even if that wallet opens multiple replay accounts.
          </div>
        ) : null}
    ...
    return (
      ...
      >
        ...
        <div className="paper-floating-product-leaderboard-copy">
          Current rank blends AI diligence with earn potential after base annual drag.
        </div>
              ...
              >
                ...
                <div className="paper-shelf-leaderboard-main">
                  ...
                  <div className="paper-shelf-leaderboard-score-row">
                    <span className="paper-shelf-score-chip">AI {row.aiScore}</span>
                    <span className="paper-shelf-score-chip">Earn {row.earnScore}</span>
                    <span className="paper-shelf-score-chip">Target {formatPercent(row.earnRate, 1)}</span>
                  </div>
                </div>
  ...
  function renderShelfLearningCard() {
    return (
      <section className="paper-shelf-learning-card">
        ...
        <div className="paper-shelf-learning-steps">
          {routeGuideSteps.map((step) => (
            <div key={`${step.number}-${step.title}`} className="paper-shelf-learning-step">
              <div className="paper-shelf-learning-step-top">
                <span className="paper-inline-step-number">{step.number}</span>
                <strong>{step.title}</strong>
              </div>
              <div className="paper-shelf-learning-step-copy">{step.detail}</div>
            </div>
          ))}
        </div>
      ...
      <header className="site-header">
        <div className="brand-wrap">
          ...
          <div>
            ...
            <div className="brand-name">{t('MSX Paper Trading Replay Lab', 'MSX 模拟交易回放实验室')}</div>
          </div>
        </div>
          ...
          <div className="paper-token-pill">
            <div className="paper-token-label">{t('Available replay cash', '可用回放资金')}</div>
            <div className="paper-token-value">{formatNotional(availableCash)} PT</div>
        ...
        <section className="card wealth-hero-card">
          <div className="section-head">
            <div>
              ...
              <h1 style={{ maxWidth: 1020 }}>{t('Practice MSX-style product decisions against historical replay bars before any live action.', '在任何真实操作之前，先用历史回放练习 MSX 风格的产品决策。')}</h1>
            </div>
            ...
            {replayAchievements.map((achievement) => (
              (() => {
                ...
                return (
                  <button
                    ...
                    className={`learn-quest-tile ${selectedRewardTaskId === achievement.id ? 'active' : ''} ${
                      tileStatus.tone === 'done' ? 'done' : tileStatus.tone === 'ready' ? 'ready' : ''
                    }`}
                ...
                {achievement.unlocked || achievement.onchainClaimed ? (
                  ...
                  <ReplayTaskPendingCover
                    taskNumber={achievement.taskNumber}
                      ...
                      return (
                        ...
                        >
                          <div className="product-top paper-shelf-product-top">
                            ...
                            {product.structureTags?.length ? (
                              <div className="paper-product-tag-row">
                                {product.structureTags.map((tag) => (
            ...
            {portfolioRows.length === 0 ? (
              <div className="reason-card">
                ...
                <div className="entry-copy">
                  ...
                  Leverage & hedging, or Strategies once the mechanics feel familiar.
                </div>
              </div>
            ) : (
