...
function WealthInner() {
  ...
  const goalFilteredProducts = useMemo(
    () => liveProducts.filter((product) => product.goals.includes(selectedGoal)),
  ...
  const baseShelfProducts = useMemo(() => {
    const bucketMatches =
      selectedCategory === 'all'
        ...
        : goalFilteredProducts.filter((product) => product.termType === selectedCategory);
        ...
        <section className="card">
          <div className="section-head">
            <div>
              ...
              <h2>{t('Start from the user goal, then reveal the product structure', '先从用户目标出发，再展示产品结构')}</h2>
            </div>
          </div>
          ...
          <div className="goal-picker">
            {GOAL_OPTIONS.map((goal) => (
              <button
                key={goal.id}
                className={`entry-card goal-card ${selectedGoal === goal.id ? 'active' : ''}`}
                onClick={() => setSelectedGoal(goal.id)}
              >
                <div className="entry-title">{goal.label}</div>
                <div className="entry-copy">{goal.description}</div>
              </button>
            ))}
          </div>
          ...
          <div className="wealth-filter-row">
            ...
            <div className="route-highlight" style={{ marginTop: 18 }}>
              <strong>{t('Recommended for this goal:', '这个目标下推荐：')}</strong> {recommendedProducts.map((product) => product.name).join(', ')}.
            </div>
          </section>
