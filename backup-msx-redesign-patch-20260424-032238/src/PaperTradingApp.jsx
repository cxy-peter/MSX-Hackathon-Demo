...
const REPLAY_ACHIEVEMENT_IDS = [6, 7];
...
const HIDDEN_REPLAY_LANE_TAB_IDS = new Set(['leverage']);
const HIDDEN_LEARNING_ROUTE_IDS = new Set(['borrow', 'routing', 'lending']);
const HIDDEN_PERP_FOCUS_IDS = new Set(['combo']);
const WEALTH_SURFACE_REPLAY_PRODUCT_IDS = new Set(['msx-stable-income', 'msx-income-ladder']);
...
function PaperTradingInner() {
  ...
  const selectedTradeAmountMaxLabel = leverageRouteActive
    ? tradeAmountMaxMode === 'ticket'
      ...
      : tradeAmountMaxMode === 'venue'
        ...
        : 'Wallet-only max'
    : 'Available cash';
  const tradeAmountFieldLabel = hedgeFocusActive ? 'Target hedge size' : 'Paper notional';
  ...
  const hedgeProtectedSleeveLabel = hedgeFocusActive
    ? hedgeHasPaperNotionalBase
      ...
      : hedgeHasMeaningfulOpenSleeve
        ...
        : 'Pick anchor + set paper notional'
    : '';
  ...
  const hedgeMetricCards = hedgeSizingReady
    ? [
        { label: 'Suggested hedge size', value: `${formatNotional(hedgeSuggestedNotional)} PT` },
        { label: 'Target hedge size', value: `${formatNotional(hedgePlannedTicketNotional)} PT` },
        { label: 'Current hedge ticket', value: `${formatNotional(hedgeLiveTicketNotional)} PT` },
        { label: 'Initial margin posted', value: `${formatNotional(routePostedBaseMarginCapital)} PT` },
        { label: 'Hedge leverage', value: routeLeverageMultiple === 1 ? 'No leverage' : `${routeLeverageMultiple}x` },
        { label: 'Target ratio', value: formatPercent(Number(hedgeRatio || 0), 0) },
        { label: 'Planned hedge ratio', value: formatPercent(hedgePlannedRatio, 1) },
        { label: hedgeRatioDisplayLabel, value: formatPercent(hedgeActualRatio, 1) },
        {
          label: 'Status',
          value: hedgeStatus,
          valueClassName:
        ...
        { label: 'Net sleeve left', value: `${formatNotional(hedgeResidualExposureValue)} PT` }
      ]
    : [];
  ...
  function updateProgressAfterTrade() {
    writeStorageJson(progressStorageKey, {
      ...progressState,
      paperTradesCompleted: Math.max(1, Number(progressState.paperTradesCompleted || 0))
    });
    setProgressState((current) => ({
      ...current,
      paperTradesCompleted: Math.max(1, Number(current.paperTradesCompleted || 0))
    }));
  }
                  ...
                  {hedgeFocusActive ? (
                    <div className="paper-inline-action-card paper-inline-action-card-compact paper-inline-hedge-frame-card">
                      <div className="paper-inline-action-head">
                        <div className="k">Hedge frame</div>
                      ...
                      <div className="paper-inline-support-list paper-inline-ticket-grid">
                        <div className="paper-inline-support-row">
                          ...
                          <strong>{hedgeProtectedSleeveLabel}</strong>
                        </div>
                        <div className="paper-inline-support-row">
                          <span>Paper notional base</span>
                          <strong>{hedgeHasPaperNotionalBase ? `${formatNotional(hedgePreviewSleeveNotional)} PT` : '--'}</strong>
                        </div>
                        <div className="paper-inline-support-row">
                          <span>Open sleeve units</span>
                          <strong>{hedgeSleeveUnitsLabel}</strong>
                        </div>
                        <div className="paper-inline-support-row">
                          <span>Live sleeve mark</span>
                          <strong>{hedgeHasMeaningfulOpenSleeve ? `${formatNotional(hedgeLiveSleeveNotional)} PT` : '--'}</strong>
                        </div>
                      </div>
                      {hedgeSizingReady ? (
                        <div className="paper-inline-hedge-progress">
                          <div className="paper-inline-hedge-progress-head">
                            ...
                            <strong>{formatNotional(hedgeProtectedSleeveNotional)} PT</strong>
                          </div>
                          ...
                          <div className="paper-inline-hedge-progress-legend">
                            ...
                            <span><i className="target" /> Target {formatNotional(hedgePlannedTicketNotional)} PT</span>
                            <span><i className="current" /> Current {formatNotional(hedgeLiveTicketNotional)} PT</span>
                          </div>
                        </div>
                      ) : null}
                      {hedgeSizingReady ? (
                        <>
                          ...
                          <div className="paper-inline-hedge-ticket-grid">
                            {hedgeMetricCards.map((card) => (
                        ...
                        <div className="paper-inline-note-box paper-inline-route-capacity-note">
                          Lock a replay bar first. Then set the paper-notional base separately from the hedge ticket so wallet cap and suggested hedge size follow the same principal.
                        </div>
                      )}
                      <div className="paper-inline-support-list">
                        <div className="paper-inline-support-row">
